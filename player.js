 ...
 * author: Zing(IT鎶€鏈畢)
 * qq: 8589561
 * motto: php鏄笘鐣屼笂鏈€濂界殑璇█(寮€涓帺绗戰煒�)
 * version: 2.2.1
 * webSite: http://www.ilt.me
 * time: 2019/03/23
 * disclaimer: 鎻掍欢淇敼浜庢槑鏈堟旦绌哄厤璐圭増锛屼粎鐢ㄤ簬瀛︿範浜ゆ祦锛屾棤鍟嗕笟浠峰€�
 *             濡傚彂鐜板晢涓氫紶鎾紝灏嗙姝㈣蒋浠剁殑鍏嶈垂浣跨敤
 */

// 妫€鏌Query鎻掍欢鏄惁瀹夎
if(typeof jQuery === 'undefined'){
    throw new Error('璇峰厛鍔犺浇jQuery鎻掍欢锛�');
}
/*mousewheel plugin*/
...
var lzxPlayerInit = function () {
    ...
    $("body").append('<div id="lzxPlayer">\n' +
        '    <div class="player">\n' +
        '        <canvas class="blur-img" width="365" height="155" id="canvas">鎮ㄧ殑娴忚鍣ㄤ笉鏀寔canvas锛岃鏇存崲楂樼骇鐗堢殑娴忚鍣紒</canvas>\n' +
        '        <div class="blur-img">\n' +
        ...
        '                    <i class="volumeup fa fa-volume-up" title="闊抽噺"></i>\n' +
    ...
    var lzxMedia = {
        play: function () {
            ...
            cicleTime = setInterval(function(){
                ...
                // 鑾峰彇宸茬紦瀛樼殑鏃堕棿  timeRanges.end(timeRanges.length - 1)
        ...
        error: function () {
            clearInterval(cicleTime);
            $player.removeClass('playing');
            lzxTips.show(songSheetList[albumId].songNames[songId] + ' - 璧勬簮鑾峰彇澶辫触锛佸皾璇曡幏鍙栦笅涓€棣�...');
            lzxMedia.next();
        },
        seeking: function () {
            lzxTips.show('鍔犺浇涓�...')
    ...
    $('.pause', $player).click(function () {
        ...
        audio.pause();
        setTimeout(function () {
            lzxTips.show("鎾斁鍣ㄤ笅娆¤闂皢鑷姩鏆傚仠");
        }, 4000);
    ...
    $('.play', $player).click(function () {
        hasgeci = true;
        $('#lzxLrc,#lzxKsc').show();
        $("li", $albumList).eq(albumId).addClass(cur).find(".artist").html("褰撳墠鎾斁 > ").parent().siblings()
            .removeClass(cur).find(".artist").html("").parent();
        startPlay();
        setTimeout(function () {
            lzxTips.show("鎾斁鍣ㄤ笅娆¤闂皢鑷姩鎾斁");
        }, 4000);
    ...
    function netmusic() {
        ...
        window.console.log(name + ' - 褰撳墠鎾斁锛�' + songSheetList[albumId].songNames[songId] + ' - '
        ...
        coverImg.error = function () {
            ...
            setTimeout(function () {
                    lzxTips.show(songSheetList[albumId].songNames[songId] + ' - 涓撹緫鍥剧墖鑾峰彇澶辫触锛�')
                },
                4000)
        };
        $cover.html(coverImg);
        if (background == 1) {
            $('.blur-img .blur', $player).attr("src", songSheetList[albumId].albumCovers[songId]); //铏氬寲鑳屾櫙
        } else {
